...
<br /><br /><br />
<?php 
	// see if the most recent move is a repeat.
	// use $cycle 
	if($json['round']<=3 && !$cycle){// echo "<p>$cycle</p>";
	?>
...
<script type="text/javascript">
	var t;
	function check_wait(){
		$.getScript(<?php echo '"' . site_url('seqsimeq/timerjs/' . $treatment_id) . '"'; ?>);
	}
	...
	// animate hovering over a button
		...
		$('button.continue_button').click(function(){
			<?php echo 'window.location = "' . site_url('seqsimeq/results_wait/' . $treatment_id) . '";';?>
			//$('.header_spacer:first').load(<?php echo "'" . site_url('seqsimeq/results_wait/' . $treatment_id . "'");?>);
		});

		$('button.review_button').click(function(){
			<?php echo 'window.location = "' . site_url('seqsimeq/review/' . $treatment_id) . '";';?>
		});
});
</script>
</body>
