...
<script type="text/javascript">
	var t;
	function check_wait(){
		$.getScript(<?php echo '"' . site_url('seqsim/timerjs/' . $treatment_id) . '"'; ?>);
	}
	...
	// animate hovering over a button
		...
		$('button.continue_button').click(function(){
			<?php echo 'window.location = "' . site_url('seqsim/results_wait/' . $treatment_id) . '";';?>
			//$('.header_spacer:first').load(<?php echo "'" . site_url('seqsim/results_wait/' . $treatment_id . "'");?>);
		});

		$('button.review_button').click(function(){
			<?php echo 'window.location = "' . site_url('seqsim/review/' . $treatment_id) . '";';?>
		});
});
</script>
</body>
