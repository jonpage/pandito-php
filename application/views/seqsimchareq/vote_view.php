 ...
 */
	...
	foreach ($json['proposed_groups'][0]['proportions'] as $agent => $value) {
		if ($agent == $agent_num) {
			...
			$proposed_payout = round(100/count($json['proposed_groups'][0]['proportions']));
...
<h2>Please vote if you would like to become a part of the proposed group</h2><br/>
 ...
 <?php suggestion_list($agent_num,$json,$num_agents,$json['current_groups'],$agent_group); ?><br />
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$('button.no_button').click(function(){
			$('.header_spacer:first').load(<?php echo "'" . site_url('seqsimchareq/send_vote/' . $treatment_id) . "/no'"?>);
		});
		$('button.yes_button').click(function(){
			$('.header_spacer:first').load(<?php echo "'" . site_url('seqsimchareq/send_vote/' . $treatment_id) . "/yes'"?>);
		});
	});
</script>
