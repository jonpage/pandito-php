 ...
 * 
 */
class Seqsimeq extends CI_Controller {
	...
	public function index($treatment_id){
		...
		if($treatment_status==-1){ // not yet in treatment
			...
			if($stage == 'wait'){
				...
				$this->load->view('seqsimeq/wait',$view_data);
			} else {
				$this->load->view('seqsimeq/instructions_view',array('treatment_id'=>$treatment_id));
			}
			...
			switch ($stage) {
				...
				case 'propose':
					if($this->session_model->get_proposer_num($session_id)==$this->session->userdata('agent_num')){
						...
						$this->load->view('seqsimeq/propose_view',array(
							'json'=>$json,
							'num_agents'=>$num_agents,
						...
						$this->load->view('seqsimeq/propose_wait_view',array(
							'json'=>$this->session_model->get_current_json($session_id),
							'num_agents'=>$num_agents,
							'session_id'=>$session_id,
							'treatment_id'=>$treatment_id,
				...
				case 'vote':
					...
					if(in_array($this->session->userdata('agent_num'), $json_array['voters'])&&!array_key_exists($this->session->userdata('agent_num'), $json_array['votes'])){
						...
						$this->load->view('seqsimeq/vote_view',array(
							'json'=>$json,
							'num_agents'=>$num_agents,
							'session_id'=>$session_id,
							'treatment_id'=>$treatment_id,
						...
						$this->load->view('seqsimeq/vote_wait_view',array(
							'json'=>$json,
							'num_agents'=>$num_agents,
							'session_id'=>$session_id,
							'treatment_id'=>$treatment_id,
				...
				case 'results':
					...
					//$search
					if($json['round']>3){
						$this->load->view('seqsimeq/results_view',array(
							'json'=>$json,
							'agent_num'=>$this->session->userdata('agent_num'),
							'cycle'=>$cycle));
					} else {
	...
	function review($treatment_id){
		...
		//	echo '<script>window.location = "' . site_url('seqsimeq/index/'.$treatment_id) . '"</script>';
		//} else { // 
			//$this->load->model('seqsim_model');
			$num_agents = $this->session_model->get_num_agents($treatment_id);
			$json = $this->session_model->get_current_json($this->session->userdata('sess_id'));
			$this->load->view('seqsimeq/review_view',array(
				'num_agents' => $num_agents,
				'session_id' => $this->session->userdata('sess_id'),
				'treatment_id' => $treatment_id,
				'agent_num'=>$this->session->userdata('agent_num'),
	...
	function show_new_groups($session_id,$coalition_id){
		$this->load->model('session_model');
		$json = $this->session_model->get_current_json($session_id);
		$this->load->view('new_groups_view_eq',array('json'=>$json,'id'=>$coalition_id));
	}
	...
	/**
	 * Either this will return a script to continue waiting or redirect to seqsimeq/index 
	...
	function waitjs($prev_stage,$treatment_id){
		...
		if($cur_stage!=$prev_stage){
			if($cur_stage == 'instructions' || $cur_stage == 'propose' || $cur_stage == 'vote' || $cur_stage == 'results'){
				echo 'window.location = "' . site_url('seqsimeq/index/' . $treatment_id) . '"';
			} else {
			...
			if($cur_stage=='wait'){ 
				...
				if($timesince>60){
					...
					echo 'window.location = "' . site_url('seqsimeq/abort') . '"';
				} else {
	...
	function instructions(){		
		...
		$template_data['main_content']='seqsimeq/instructions_view';
	...
	*/
	...
	function finish_instructions($session_id){
		...
		redirect('seqsimeq/index/'.$this->session->userdata('treatment_id'));
	}
	...
	function results_wait($treatment_id){
		...
		if($this->session_model->get_stage($this->session->userdata('sess_id'))!='results'){
			...
			$num_agents = $this->session_model->get_num_agents($treatment_id);
			$json = $this->session_model->get_current_json($this->session->userdata('sess_id'));
			$this->load->view('seqsimeq/results_wait_view',array(
				'num_agents' => $num_agents,
				'session_id' => $this->session->userdata('sess_id'),
				'treatment_id' => $treatment_id,
				'agent_num'=>$this->session->userdata('agent_num'),
...
//			echo "alert('$treatment_id');";
			...
			if(strpos($finished, "0")===false){ // ready for next stage
				$this->load->model('seqsim_model');
				$this->seqsim_model->start_treatment($treatment_id);
				echo 'window.location = "' . site_url('seqsimeq/finish_instructions/'.$this->session->userdata('sess_id')) . '";';
				//echo 'window.location = "' . site_url('seqsimeq/index/' . $treatment_id) . '"';
			} else {
